...
- **Read-only defaults.** Most tools are read-only. Write-capable tools: Mail (send, draft), Teams (send message), Planner (create/update tasks), To Do (create/update), OneDrive (create folder, generate link), Confluence (create/update pages and comments), Entra (dismiss risky user), Obsidian (read/write notes).
