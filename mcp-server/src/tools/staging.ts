...
export function registerStagingTools(server: McpServer, enabled: boolean): void {
  ...
  server.registerTool(
    ...
    async ({ file, limit }: { file: StagingFile; limit?: number }) => {
      try {
        const latest = latestStagingDir();
  ...
  server.registerTool(
    ...
    async ({ job }: { job?: "graph" | "ninjaone" | "wazuh" | "unifi" | "planner" }) => {
      try {
  ...
  server.registerTool(
    ...
    async ({ device_id, days }: { device_id: string; days: number }) => {
      try {
  ...
  server.registerTool(
    ...
    async ({ source, days }: { source: "ninjaone" | "wazuh" | "unifi"; days: number }) => {
      try {
        const dbPath = path.join(getDbDir(), "metrics.db");
        if (!fs.existsSync(dbPath)) {
          return ok({ error: "Metrics database not found — run collector gather first." });
  ...
  server.registerTool(
    ...
    async ({ threshold_pct }: { threshold_pct: number }) => {
      try {
        const dbPath = path.join(getDbDir(), "metrics.db");
        if (!fs.existsSync(dbPath)) {
          return ok({ error: "Metrics database not found — run collector gather first." });
