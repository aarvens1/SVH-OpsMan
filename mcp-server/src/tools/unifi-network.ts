...
export function registerUnifiNetworkTools(server: McpServer, enabled: boolean): void {
  ...
  server.registerTool(
    ...
    async ({ site_id, name, vlan, subnet, dhcp_enabled, dhcp_start, dhcp_stop }) => {
      ...
      try {
        ...
        if (dhcp_start) body["dhcpd_start"] = dhcp_start;
        if (dhcp_stop) body["dhcpd_stop"] = dhcp_stop;

        const res = await createControllerClient().post(
          `/api/v2/sites/${site_id}/networks`,
          body
        );
        return ok(res.data);
      } catch (e) {
  ...
  server.registerTool(
    ...
    async ({ site_id, name, action, direction, protocol, enabled: ruleEnabled, src_address, dst_address, dst_port }) => {
      ...
      try {
        ...
        if (src_address) body["src_address"] = src_address;
        if (dst_address) body["dst_address"] = dst_address;
  ...
  server.registerTool(
    ...
    async ({ site_id, device_id }) => {
      ...
      try {
        const res = await createControllerClient().post(
          `/api/v2/sites/${site_id}/devices/${device_id}/restart`,
          { reboot_type: "soft" }
        );
  ...
  server.registerTool(
    ...
    async ({ site_id, mac, block }) => {
      ...
      try {
        ...
        const res = await createControllerClient().post(
          `/api/v2/sites/${site_id}/clients/${mac}/${action}`,
          { mac }
        );
