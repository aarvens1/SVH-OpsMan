...
const DISABLED_MSG = "Defender for Endpoint not configured: set MDE_TENANT_ID, MDE_CLIENT_ID, MDE_CLIENT_SECRET. " +
...
export function registerDefenderMdeTools(server, enabled) {
    server.registerTool("mde_list_devices", {
        description: "List all devices enrolled in Microsoft Defender for Endpoint. " +
        ...
        try {
            ...
            if (health_status !== "all")
                filters.push(`healthStatus eq '${health_status}'`);
            if (risk_score !== "all")
                filters.push(`riskScore eq '${risk_score}'`);
    ...
    server.registerTool("mde_get_device", {
        description: "Get full details for a specific Defender-enrolled device: OS version, IP addresses, " +
    ...
    server.registerTool("mde_get_device_vulnerabilities", {
        description: "List CVEs (vulnerabilities) found on a specific Defender-enrolled device, " +
        ...
        try {
            ...
            const params = { $top: top };
            if (severity !== "all")
                params["$filter"] = `severity eq '${severity}'`;
            const res = await mdeClient(token).get(`/machines/${machine_id}/vulnerabilities`, { params });
            return ok(res.data);
        }
        catch (e) {
            return err(e);
    ...
    server.registerTool("mde_run_av_scan", {
        description: "Trigger an antivirus scan on a Defender-enrolled device. " +
        ...
        try {
            const token = await getMdeToken();
            const res = await mdeClient(token).post(`/machines/${machine_id}/runAntiVirusScan`, { scanType: scan_type, comment });
            return ok(res.data);
        }
        catch (e) {
            return err(e);
    ...
    server.registerTool("mde_isolate_device", {
        description: "Isolate a device from the network to contain a potential breach. " +
        ...
        inputSchema: z.object({
            ...
            isolation_type: z
                .enum(["Full", "Selective"])
                .default("Full")
                .describe("'Full' blocks all traffic; 'Selective' allows Outlook and Teams through for communication"),
    ...
    server.registerTool("mde_unisolate_device", {
        description: "Remove a device from network isolation in Defender for Endpoint, restoring full connectivity.",
        inputSchema: z.object({
            machine_id: z.string().describe("MDE machine ID"),
    ...
    server.registerTool("mde_list_alerts", {
        description: "List Defender for Endpoint alerts with severity, status, affected device, and assigned investigator.",
        inputSchema: z.object({
            severity: z
    ...
    server.registerTool("mde_list_indicators", {
        description: "List custom threat indicators (IOCs) configured in Defender for Endpoint: " +
        ...
        try {
            ...
            if (indicator_type !== "all")
                filters.push(`indicatorType eq '${indicator_type}'`);
            if (action !== "all")
    ...
    server.registerTool("mde_create_indicator", {
        description: "Create a custom threat indicator (IOC) in Defender for Endpoint to block or alert on " +
        ...
        inputSchema: z.object({
            ...
            action: z
                ...
                .describe("'AlertAndBlock' generates an alert and blocks the IOC; 'Block' silently blocks; 'Allowed' whitelists"),
        ...
        try {
            ...
            if (expiry_time)
                body["expirationTime"] = expiry_time;
    ...
    server.registerTool("mde_get_security_recommendations", {
        description: "List Defender TVM (Threat & Vulnerability Management) security recommendations " +
...
//# sourceMappingURL=defender-mde.js.map