...
export function registerPlannerTools(server, enabled) {
    // ── Plans ──────────────────────────────────────────────────────────────────
    server.registerTool("planner_list_plans", {
        description: "List all Planner plans belonging to a Microsoft 365 group. Returns plan IDs, titles, and owner.",
    ...
    server.registerTool("planner_create_plan", {
    ...
    server.registerTool("planner_list_tasks", {
        description: "List tasks in a plan. Optionally scope to a specific bucket. Returns title, assignments, due date, percent complete.",
        inputSchema: z.object({
            plan_id: z.string().describe("The Planner plan ID"),
    ...
    server.registerTool("planner_get_task", {
        description: "Get a Planner task's basic fields: title, due date, percent complete, assignments, bucket, and the @odata.etag needed for updates.",
        inputSchema: z.object({
    ...
    server.registerTool("planner_create_task", {
        description: "Create a new Planner task. Optionally assign it, set a due date, and add initial checklist items. " +
        ...
        try {
            const token = await getGraphToken(GRAPH_SCOPE);
            const client = graphClient(token);
            const body = { planId: plan_id, bucketId: bucket_id, title };
            if (due_date)
                body["dueDateTime"] = due_date;
            if (assigned_to)
                body["assignments"] = { [assigned_to]: { "@odata.type": "#microsoft.graph.plannerAssignment", orderHint: " !" } };
            ...
            if (checklist_items && checklist_items.length > 0) {
                ...
                const detailsEtag = detailsRes.headers["etag"] ?? detailsRes.data["@odata.etag"];
                const checklist = {};
    ...
    server.registerTool("planner_update_task", {
        description: "Update a Planner task's title, due date, percent complete (0–100), or bucket. " +
    ...
    server.registerTool("planner_delete_task", {
        description: "Delete a Planner task. Requires the etag from planner_get_task (@odata.etag field).",
        inputSchema: z.object({
            task_id: z.string().describe("The Planner task ID"),
    ...
    server.registerTool("planner_get_task_details", {
        description: "Get a task's notes/description and full checklist. " +
    ...
    server.registerTool("planner_update_task_notes", {
        description: "Update the notes/description field of a Planner task. " +
        ...
        try {
            const token = await getGraphToken(GRAPH_SCOPE);
            const res = await graphClient(token).patch(`/planner/tasks/${task_id}/details`, { description }, { headers: { "If-Match": etag } });
    ...
    server.registerTool("planner_update_checklist_item", {
        description: "Add, update (check/uncheck/rename), or remove a checklist item on a Planner task. " +
        ...
        try {
            ...
            const res = await graphClient(token).patch(`/planner/tasks/${task_id}/details`, { checklist }, { headers: { "If-Match": etag } });
