...
export function registerEntraAdminTools(server, enabled) {
    server.registerTool("entra_get_user_mfa_methods", {
        description: "List the MFA / authentication methods registered for a user: " +
        ...
        try {
            ...
            const res = await graphClient(token).get(`/users/${user_id}/authentication/methods`);
            return ok(res.data);
        }
        catch (e) {
            return err(e);
    ...
    server.registerTool("entra_list_conditional_access_policies", {
        description: "List all Conditional Access policies in the tenant, including their state (enabled/disabled/report-only), " +
        ...
        try {
            ...
            const res = await graphClient(token).get("/identity/conditionalAccess/policies", { params });
            return ok(res.data);
        }
        catch (e) {
            return err(e);
    ...
    server.registerTool("entra_list_app_registrations", {
        description: "List all Entra ID app registrations. Returns display name, app ID, creation date, " +
        ...
        try {
            ...
            const params = {
                $top: top,
                $select: "id,displayName,appId,createdDateTime,passwordCredentials,keyCredentials,signInAudience",
            };
            if (filter)
    ...
    server.registerTool("entra_list_expiring_secrets", {
        description: "Find app registrations and service principals with client secrets or certificates " +
        ...
        try {
            ...
            check(appsRes.data.value, "app_registration");
            check(spsRes.data.value, "service_principal");
            expiring.sort((a, b) => {
    ...
    server.registerTool("entra_list_directory_roles", {
        description: "List all active directory roles in the tenant (e.g. Global Administrator, " +
    ...
    server.registerTool("entra_get_role_members", {
        description: "List the users, groups, and service principals currently assigned to a directory role. " +
    ...
    server.registerTool("entra_list_risky_users", {
        description: "List users flagged as risky by Entra ID Identity Protection, with their risk level " +
        ...
        try {
            ...
            if (risk_level !== "all")
                params["$filter"] = `riskLevel eq '${risk_level}'`;
    ...
    server.registerTool("entra_dismiss_risky_user", {
        description: "Dismiss the risk for one or more users in Entra ID Identity Protection, " +
...
//# sourceMappingURL=entra-admin.js.map