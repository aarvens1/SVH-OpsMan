...
1. Send a DM to Aaron Stevens (`user.upn`) via `teams_send_message`. First use `teams_list_my_chats` to find the 1:1 self-chat (Teams "Chat with yourself" / "Notes to self") — use that chat ID. If no self-chat is found, pick the first 1:1 chat in the list as a test target and note which chat was used in the incident note. Bare facts only: what, which systems, assigned owner. Write the message in Aaron's voice — no greeting, no exclamation marks, plain declarative sentences, one bold imperative if immediate action is required (e.g. `**Do not log in to <system> until further notice.**`).

   **DM routing is active** — this skill currently routes all Teams sends to a DM rather than the IT techs channel. This is for workflow validation. Once sends are confirmed working, update this step to target the IT techs channel (`groups.it_team` from config).
...
2. Draft a Teams notification and Planner card — present both to the operator for review. Do not send. Draft the Teams message in Aaron's voice (same guidance as Burning Building above). Run the self-check before presenting.
...
1. **IOC enrichment** — look up any IP, domain, hash, or email in `mde_list_indicators`. If the IOC isn't in Defender, note that no internal intel is available.
...
After writing the incident note, append one line to `System/skill-log.md` in the vault:
`YYYY-MM-DD HH:MM | ir-triage | Incidents/Active/YYYY-MM-DD-[name].md | [one-line summary of the triage result]`
