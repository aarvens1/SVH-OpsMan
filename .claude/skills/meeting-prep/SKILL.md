...
- `planner_get_user_tasks` (user_id: `config.user.upn`, open_only: true) — Aaron's open tasks relevant to this meeting's topic. For tasks involving non-Aaron attendees, use `planner_list_tasks` across known operational boards and filter by attendee name in titles or notes — app auth cannot query tasks by arbitrary user ID.
